<template>
    <v-container>
        ...
        <v-list class="my-4" style="max-height: 600px; overflow-y: auto;">
            <v-list-item v-for="(transaction, index) in transactions" :key="index" class="mb-16 list_item" :style="index == 0 ? 'border-top: 5px solid rgba(0, 0, 0, 0.12)' : ''">
                <v-row v-if="index == 0" class="mt-6 mb-6">
                    ...
                    <v-col cols="7">
                        <v-list-item-content>
                            ...
                            <div>Quantity: {{ transaction.quantity }}
                            function(transaction.transaction_detail)</div>
                        </v-list-item-content>
                        return len(Transaction_detail)
                    </v-col>
                    <v-col>
                        <v-list-item-action>
                            <v-btn class="mt-8" color="primary" @click="goToDetailsPage(transaction)">View Details</v-btn>
                        </v-list-item-action>
                    </v-col>
                </v-row>
